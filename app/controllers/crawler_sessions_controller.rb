class CrawlerSessionsController < ApplicationController
  before_action :set_crawler_session, only: [:edit, :update, :destroy]
  ...
  def show
    previous_id = params[:id].to_i - 1
    @previous = CrawlerSession.includes(:url_history).find_by id: previous_id
    @crawler_session =  CrawlerSession.includes(:url_history).find_by id: params[:id]
  end
