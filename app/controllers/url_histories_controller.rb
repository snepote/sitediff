class UrlHistoriesController < ApplicationController
  ...
  def index
    @url_histories = UrlHistory.paginate(page: params[:page])
  end
  ...
  private
    ...
    def set_url_history
      @url_history = UrlHistory.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def url_history_params
      params.require(:url_history).permit(:url_id, :crawler_session_id, :screenshot_id, :diff_to_previous)
    end
end
